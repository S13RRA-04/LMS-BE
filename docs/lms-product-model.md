...
MongoDB is now the runtime repository for portal settings, departments, courses, enrollments, internal users, and audit logs. The API creates the required staging collections and indexes in the `LMS` database with the `staging_` prefix in development and test.
